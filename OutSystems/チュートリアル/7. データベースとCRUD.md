...
３．作成したServer Actionを「CallGeaspi」にRenameします。CallGeaspiのアクション編集画面にて、先程切り取ったGetAdressを ```Ctl+P / Cmd+P``` で貼り付けて、フローのStartとEndの間に配置します。
...
４．Interfaceタブを選択し、Module → UI Flows → Main Flow → Mapの「OnClick」をダブルクリックしてボタンアクションの編集画面を表示します。既存のフローの中にあるStartとListAppendの間にLogicタブから「CallGeaspi」をドラッグ＆ドロップで配置します。
